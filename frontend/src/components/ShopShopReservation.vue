<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            ShopReservation # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            ShopReservation
        </v-card-title >        
        ...
        <v-card-text>
            <Number v-if="editMode" label="ShopId" v-model="value.shopId" :editMode="editMode" :inputUI="''"/>
            <Date label="ReservedDate" v-model="value.reservedDate" :editMode="editMode" :inputUI="''"/>
            <Date label="ReservedTime" v-model="value.reservedTime" :editMode="editMode" :inputUI="''"/>
            <Number label="ReservedCnt" v-model="value.reservedCnt" :editMode="editMode" :inputUI="''"/>
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/shopReservations'), this.value)
                        } else {
